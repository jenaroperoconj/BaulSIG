...
class ExploradorAdmin:
    def __init__(self, master, modo_oscuro=False):
    ...
    def _crear_widgets(self):
        ...
        self.drop_area = tk.Label(self.panel_der, text="⬇ Arrastra archivos aquí para subirlos",
                                  bg="#2a2a2a", fg="#aaaaaa", font=("Segoe UI", 10),
                                  relief="ridge", height=3)
        self.drop_area.pack(fill="x", padx=10, pady=(0, 10))

        self.drop_area.drop_target_register(DND_FILES)
        self.drop_area.dnd_bind('<<Drop>>', self._procesar_drop)
    ...
    def _procesar_drop(self, event):
        ...
        for path in paths:
            ...
            if ruta_origen.is_file():
                destino = self.ruta_actual / ruta_origen.name
                if not destino.exists():
                    shutil.copy2(ruta_origen, destino)
                    registrar_archivo(ruta_origen.name, str(self.ruta_actual.relative_to(self.BASE_DIR)), 1, es_carpeta=False)
                    archivos_subidos += 1
            elif ruta_origen.is_dir():
                for subpath in ruta_origen.rglob("*"):
                    if subpath.is_file():
                        rel = subpath.relative_to(ruta_origen)
                        destino = self.ruta_actual / rel
                        destino.parent.mkdir(parents=True, exist_ok=True)
                        shutil.copy2(subpath, destino)
                        registrar_archivo(destino.name, str(destino.parent.relative_to(self.BASE_DIR)), 1, es_carpeta=False)
                        archivos_subidos += 1

        self._actualizar_tabla(self.ruta_actual)
        messagebox.showinfo("Carga completada", f"Se subieron {archivos_subidos} archivo(s).")
    ...
    def _renombrar(self, ruta, item, es_arbol):
        ...
        if nuevo:
            ...
            try:
                ...
                if archivo_id:
                    registrar_log(usuario_id, archivo_id, tipo, motivo_log, nombre_original)
    ...
    def _eliminar(self, ruta):
        ...
        try:
            ...
            if es_carpeta:
                ...
                for subelemento in ruta.rglob("*"):
                    if subelemento.is_file() or subelemento.is_dir():
                        ...
                        if archivo_id:
                            registrar_log(usuario_id, archivo_id, tipo, "Eliminado dentro de carpeta eliminada", nombre_sub)
            ...
            if archivo_id:
                ...
                registrar_log(usuario_id, archivo_id, tipo, "Elemento eliminado", nombre_original)
    ...
    def _subir_archivo(self):
        ...
        if archivo:
            ...
            try:
                ...
                usuario_id = 1  # <- será dinámico más adelante

                archivo_id = registrar_archivo(nombre_archivo, ruta_relativa, usuario_id, es_carpeta=False)
                if archivo_id:
                    registrar_log(usuario_id, archivo_id, "subido", "Archivo cargado al sistema", nombre_archivo)
    ...
    def _crear_carpeta(self, donde):
        ...
        if nombre:
            ...
            try:
                ...
                usuario_id = 1  # <- luego reemplazar con sesión actual
                archivo_id = registrar_archivo(nombre, ruta_relativa, usuario_id, es_carpeta=True)

                if archivo_id:
                    registrar_log(usuario_id, archivo_id, "carpeta_creada", "Carpeta creada manualmente", nombre)
        ...
        def mover():
            ...
            try:
                ...
                if archivo_id:
                    ...
                    registrar_log(usuario_id, archivo_id, "movido",
...
def abrir_menu_admin(user_id, modo_oscuro):
    root = TkinterDnD.Tk()  # ← NO usar tk.Tk()
    app = ExploradorAdmin(root, modo_oscuro=modo_oscuro)
    root.mainloop()