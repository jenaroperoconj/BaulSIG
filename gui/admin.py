...
class ExploradorAdmin:
    ...
    def _actualizar_tabla(self, ruta: Path):
        ...
        if self.orden_columna == "Nombre":
            ...
            elementos.sort(key=lambda x: x[0].lower())
    ...
    def _descargar(self, ruta):
        ...
        label = tk.Label(ventana, text="", font=("Segoe UI", 9))
        label.pack()
        ...
        def actualizar_label(event):
            ...
            if seleccion == "Solicitud":
                label.config(text="Número de solicitud:")
            elif seleccion == "Trabajo interno":
                label.config(text="Nombre del trabajo:")
            elif seleccion == "Modificación de datos":
                label.config(text="Justifica la modificación:")
            elif seleccion == "Otro motivo":
                label.config(text="Describe el motivo exacto:")
        ...
        def realizar_descarga():
            ...
            if not tipo or not detalle:
                messagebox.showerror("Faltan datos", "Debes completar el motivo y detalle.")
                return
            ...
            destino = filedialog.asksaveasfilename(defaultextension="", initialfile=nombre_archivo)
            if not destino:
                return
            ...
            try:
                ...
                cur.execute("""
                    ...
                    VALUES (%s, %s, %s, 'descargado', %s)
                """, (self.user_id, archivo_id, motivo_final, datetime.now()))
                conn.commit()
                conn.close()

                messagebox.showinfo("Descargado", "El archivo fue descargado y registrado.")
                ventana.destroy()
